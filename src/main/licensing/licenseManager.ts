...
const secureStore = new Store({
  ...
  encryptionKey: process.env.LICENSE_ENCRYPTION_KEY || '6d24f9b2d334e2095f93b7de9b63df751650956b9e74378d727d163216b673fd'
});
...
export class LicenseManager {
  ...
  public getDeviceId(): string {
    try {
      ...
      const fallbackId = crypto
        .createHash('sha256')
        .update(os.hostname() + os.platform() + os.arch() + os.cpus()[0].model + APP_SALT)
        .digest('hex');
  ...
  public async checkTrialStatus(): Promise<boolean> {
    try {
      ...
      try {
        ...
        const response = await axios.post(`${SUPABASE_API_URL}/checkTrialStatus`, {
          deviceId: this.deviceInfo.deviceId
        }, {
          headers: {
            'x-environment': ACTIVE_ENVIRONMENT,
  ...
  private handleLocalTrialCheck(localTrialInfo: TrialInfo | null): boolean {
    try {
      // Wenn wir keine Trial-Informationen haben, starten wir einen neuen Trial
      if (!localTrialInfo) {
  ...
  public async activateDevice(licenseKey: string): Promise<boolean> {
    try {
      const response = await axios.post(`${SUPABASE_API_URL}/activateDevice`, {
        licenseKey,
        deviceId: this.deviceInfo.deviceId,
  ...
  public async deactivateDevice(licenseKey: string): Promise<boolean> {
    try {
      ...
      const response = await axios.post(`${SUPABASE_API_URL}/deactivateDevice`, {
        licenseKey,
        deviceId,
        deviceName
      }, {
   ...
   */
  public async deactivateSpecificDevice(licenseKey: string, deviceId: string): Promise<boolean> {
    try {
      const response = await axios.post(`${SUPABASE_API_URL}/deactivateDevice`, {
        licenseKey,
        deviceId
  ...
  public async getActivatedDevices(licenseKey: string): Promise<any[]> {
    try {
      const response = await axios.post(`${SUPABASE_API_URL}/getActivatedDevices`, {
        licenseKey
      }, {
        headers: {
          'x-environment': ACTIVE_ENVIRONMENT,
  ...
  // Die getTrialInfo-Methode ist weiter unten mit erweiterter Funktionalität implementiert
  ...
  public isInTrial(): boolean {
    ...
    return trialInfo ? trialInfo.isTrialActive : false;
  }
  ...
  public getTrialInfo(): TrialInfo | null {
    try {
      const storedTrialInfo = secureStore.get('trialInfo') as TrialInfo | undefined;
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        const trialInfo: TrialInfo = {
          ...
          remainingDays: this.calculateRemainingDays(response.data.trial_end_date),
          email: email
        };
        ...
        dialog.showMessageBox({
          ...
          message: `Ihre 7-tägige Testversion wurde erfolgreich aktiviert und läuft bis zum ${new Date(trialInfo.trialEndDate).toLocaleDateString()}.`,
          buttons: ['OK']
        });
        ...
        throw new Error(response.data?.error || 'Unbekannter Fehler bei der Trial-Aktivierung');
      }
    } catch (error) {
  ...
  private showOfflineGracePeriodExpiredDialog(): void {
    dialog.showMessageBox({
      ...
      message: 'Die Offline-Gnadenfrist ist abgelaufen. Bitte stellen Sie eine Internetverbindung her, um Ihre Lizenz zu verifizieren.',
      buttons: ['OK']
    });
  }
  ...
  private showMaxDevicesReachedDialog(): void {
    dialog.showMessageBox({
      ...
      message: 'Sie haben die maximale Anzahl an Geräten (3) für diese Lizenz erreicht. Bitte deaktivieren Sie ein anderes Gerät, um dieses Gerät zu aktivieren.',
      buttons: ['OK']
    });
  }
  ...
  private showActivationFailedDialog(): void {
    dialog.showMessageBox({
      ...
      message: 'Die Aktivierung Ihres Geräts ist fehlgeschlagen. Bitte versuchen Sie es später erneut oder kontaktieren Sie den Support.',
      buttons: ['OK']
    });
  }
   ...
   */
  public async activateLicenseFromSession(sessionId: string, environment: string = ACTIVE_ENVIRONMENT): Promise<boolean> {
    try {
      console.log(`[License] Aktiviere Lizenz aus Stripe Session: ${sessionId}`);
      
      // Anfrage an den Server senden, um die Lizenz zu aktivieren
      const response = await axios.post(`${SUPABASE_API_URL}/activateLicenseFromSession`, {
        sessionId,
        deviceId: this.deviceInfo.deviceId,
        deviceName: this.deviceInfo.deviceName
      }, {
      ...
      if (data.success && data.licenseKey) {
        ...
        dialog.showMessageBox({
          ...
          message: 'Ihre Lizenz wurde erfolgreich aktiviert. Vielen Dank für Ihren Kauf!',
          buttons: ['OK']
        });
        
        return true;
      } else {
        console.error('[License] Fehler bei der Lizenzaktivierung:', data.message || 'Unbekannter Fehler');
        this.showActivationFailedDialog();
        return false;
      }
    } catch (error) {
  ...
  private showCheckoutErrorDialog(): void {
    dialog.showMessageBox({
      ...
      message: 'Der Stripe Checkout konnte nicht geöffnet werden. Bitte versuchen Sie es später erneut oder kontaktieren Sie den Support.',
      buttons: ['OK']
    });
  }
