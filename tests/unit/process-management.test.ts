...
describe("Process Management", () => {
  ...
  describe("Prozess-Identifikation", () => {
    ...
    it("sollte Prozess anhand des Titel-Patterns finden", () => {
      ...
      // Act
      const editorProcesses = findProcessesByTitlePattern(processes, "* - Editor");
  ...
  describe("Prozess-Gruppen-Management", () => {
    ...
    it("sollte Browser-Subprozesse identifizieren", () => {
      ...
      expect(browserProcesses.subprocesses.map(p => p.id)).toEqual([5678, 9012]);
    });
...
function buildProcessTree(processes: any[]): any[] {
  ...
  processes.forEach(process => {
    if (process.parentId && processMap.has(process.parentId)) {
      ...
      const child = processMap.get(process.id);
      parent.children.push(child);
    } else {
      rootProcesses.push(processMap.get(process.id));
    }
  });
...
function getProcessHierarchy(processes: any[], processId: number): any[] {
  ...
  while (currentProcess) {
    hierarchy.unshift(currentProcess);
    currentProcess = currentProcess.parentId ? processMap.get(currentProcess.parentId) : null;
  }
