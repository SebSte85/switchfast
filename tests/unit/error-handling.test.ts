...
describe("Error Handling & Edge Cases", () => {
  ...
  describe("Speicher- und Performance-Grenzen", () => {
    ...
    it("sollte Speicher-Limits respektieren", () => {
      ...
      // Act & Assert
      expect(() =>
        executeWithMemoryLimit(memoryIntensiveOperation, 50 * 1024 * 1024)
    ...
    it("sollte gleichzeitige Operationen limitieren", async () => {
      ...
      const concurrentOperations = Array.from(
        { length: 100 },
        (_, i) => () => Promise.resolve(`Operation ${i}`)
      );

      // Act
      const result = await executeConcurrentlyWithLimit(
  ...
  describe("Netzwerk- und Datenbank-Fehler", () => {
    ...
    it("sollte Rate-Limiting handhaben", async () => {
      ...
      expect(result.userMessage.toLowerCase()).toContain("zu viele anfragen");
    });
  ...
  describe("Benutzer-Input-Validierung", () => {
    ...
    it("sollte XSS-Versuche in Theme-Namen abfangen", () => {
      ...
      maliciousNames.forEach((name) => {
        expect(() => validateThemeName(name)).toThrow();
      });
    });
    ...
    it("sollte extreme Eingabegrößen handhaben", () => {
      ...
      expect(() =>
        validateApplicationList(extremeInputs.tooManyApplications)
      ).toThrow("Zu viele Anwendungen");
    });
  });
...
async function handlePowerShellError(error: any) {
  ...
  if (
    error.stderr?.includes("UnauthorizedAccessException") ||
    ...
    error.message?.includes("PowerShell not found")
  ) {
    errorType = "POWERSHELL_NOT_FOUND";
    userMessage = "PowerShell nicht verfügbar - Fallback aktiv";
  }
...
function repairCorruptThemes(themes: any[]) {
  return themes.map((theme) => {
    ...
    if (!repaired.shortcut) {
      repaired.shortcut = `Alt+${Math.random()
        .toString(36)
        .substr(2, 1)
        .toUpperCase()}`;
    }
...
function validateThemeName(name: string) {
  ...
  for (const pattern of maliciousPatterns) {
    if (name.toLowerCase().includes(pattern.toLowerCase())) {
      throw new Error("Ungültige Zeichen im Theme-Namen");
    }
  }
